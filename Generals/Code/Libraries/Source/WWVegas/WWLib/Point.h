...
class TPoint3D : public TPoint2D<T> 
{
	...
	public:
		...
		// Vector support functions.
		T Length(void) const {return(T( std::sqrt( TPoint2D<T>::X * TPoint2D<T>::X + TPoint2D<T>::Y * TPoint2D<T>::Y + Z * Z)));}
		TPoint3D<T> const Normalize(void) const 
		{
			double len = std::sqrt( TPoint2D<T>::X * TPoint2D<T>::X + TPoint2D<T>::Y * TPoint2D<T>::Y + Z * Z );
			if (len != 0.0) 
			{
