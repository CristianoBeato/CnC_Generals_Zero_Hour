...
class DSurface : public XSurface
{
	...
	public:
		...
		//
		DSurface(int width, int height, bool system_memory = false, SDL_PixelFormat* pixform = nullptr );

		//
		//	Creates a surface from a previously created DirectDraw surface object.
		//
		DSurface( SDL_Surface surfaceptr );

		//
		// Get/Release a windows device context from a DirectX surface
		//
		SDL_Renderer* GetDC(void);
		int ReleaseDC( SDL_Renderer* hdc );
...
#if 0
		//
		// SDL3 renderer specific data.
		//
		SDL_Renderer*				Renderer;
		SDL_Texture*				SurfaceTexture;
		SDL_Rect*					SurfaceRect; 
		static SDL_PixelFormat		PixelFormat; //
		static SDL_Rect* 			Clipper; //	Pointer to the clipper object that is attached to the primary surface.
#endif
